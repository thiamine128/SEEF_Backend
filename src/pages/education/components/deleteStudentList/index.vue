<template>
    <div class="approve-dialog">
        ...
        <div class="dialog-footer">
            ...
            <button @click="deleteT">删除</button>
        </div>
    </div>
</template>
...
import {deleteStudentAPI} from "@/pages/education/components/deleteStudentList/api/api";

export default {
    name: "chosenStudentList",
    data() {
        return {
            show: true,
            page: 1,
            ...
            for(let i = 0; i < this.applicants.length; i++) {
                for(let j = 0; j < this.selectedStudents.length; j++) {
                    if(this.applicants[i].name === this.selectedStudents[j]) {
                        students.push({"studentId" :this.applicants[i].id, "classId": this.classId});
                    }
                }
            }
            // console.log(students);
            await deleteStudentAPI(students);
            this.selectedStudents = [];
            this.$emit('update-parent-chosen', false);
        },
        close() {
            this.$emit('update-parent-chosen', false);
        }
